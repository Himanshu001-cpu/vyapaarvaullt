...
export function SuppliersPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    const success = await createParty({ name: newName, type: 'supplier' });
    if (success) {
      toast({ title: 'Supplier Created', description: `${newName} has been added.` });
      setNewName('');
      setIsCreating(false);
      loadParties('supplier');
    }
  ...
  if (error) return <ErrorState message={error} retry={() => loadParties('supplier')} />;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-xl border bg-card text-card-foreground shadow">
        {parties.length === 0 ? (
           <EmptyState message="No suppliers found. Add your first supplier above." icon={Users} />
        ) : (
