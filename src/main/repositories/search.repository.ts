...
export class SearchRepository {
  static async globalSearch(query: string, limit = 10): Promise<GlobalSearchResult[]> {
    ...
    const partyResults = await db
      ...
      .where(and(isNull(parties.deleted_at), sql`parties_fts MATCH ${query}`))
      .limit(limit);
    ...
    const productResults = await db
      ...
      .where(and(isNull(products.deleted_at), sql`products_fts MATCH ${query}`))
      .limit(limit);
    ...
    const txnResults = await db
      ...
      .where(and(isNull(transactions.deleted_at), sql`transactions_fts MATCH ${query}`))
      .limit(limit);
