...
export class InvoiceRepository {
  ...
  static async getNextInvoiceNumber(tx: any = db): Promise<string> {
    const [result] = await tx.select({ maxNumber: sql<string>`MAX(${invoices.invoice_number})` }).from(invoices);
    if (!result || !result.maxNumber) {
        return 'INV-0001';
    }
    const match = result.maxNumber.match(/INV-(\d+)/);
    if (match) {
