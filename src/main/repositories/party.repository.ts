...
export class PartyRepository {
  ...
  static async getBalance(partyId: number) {
    ...
    const [invoiceResult] = await db
      .select({
        totalInvoiced: sql<number>`SUM(${invoices.total_amount})`.mapWith(Number)
      })
      .from(invoices)
    ...
    const totalInvoiced = invoiceResult?.totalInvoiced || 0;
  ...
  static async searchFTS(query: string, type?: 'customer' | 'supplier' | 'both', limit = 20) {
    ...
    conditions.push(isNull(parties.deleted_at));
    conditions.push(sql`parties_fts MATCH ${sanitized}`);
